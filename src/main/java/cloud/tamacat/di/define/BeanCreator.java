...
public class BeanCreator {
	...
	public synchronized <T> T getBean(String id, Class<T> type) {
		...
		if (def.isSingleton()) { //singleton instance
	 ...
	 * @return instance
	 */
	<T> T initializeInstance(BeanDefine def, T instance) {
	...
	<T extends Object> T newInstance(BeanDefine def) {
		T instance = null;
		//for Factory Method.
		...
		for (BeanDefineParam prop : def.getPropertyList()) {
            ...
            if (prop.isRef()) {
            	...
            	if (param == null) {
                	throw new ClassUtilsException("ref="+prop.getRefId() + " param is null. [" + prop.getRefId() + ":" + prop.getParamType() + "] in " + instance);
            	}
	 ...
	 * @return new instance
	 */
	Object newInstanceForConstructorInjection(BeanDefine def) {
	 ...
	 * @return instance
	 */
	<T> T setterInjection(T instance, BeanDefineParam prop, BeanDefine def, Object param) {
		...
		if (method == null) {
            ...
            if (method == null) {
            	throw new ClassUtilsException("id="+def.getId()+" method or param is null. [" + prop.getName() + ":" + param + "] in " + instance);
            }
        }
	 ...
	 */
	@SuppressWarnings("unchecked")
	<T extends Object> T setterInjectionByBasicType(
